...
const createProduct = async (event) => {
  ...
  try {
    ...
    // set default publish flag
    productRequest.publish = false;
    // Get the current date and time using moment.js
    const moment = require('moment');
    const currentDateAndTime = moment().format('DD-MM-YYYY HH:mm:ss');
    productRequest.createdDate = currentDateAndTime;
    productRequest.modifiedDate = currentDateAndTime;
    ...
    // const catlogRequest = JSON.parse(event.body);
    // catlogRequest.id = productId;
    // catlogRequest.publish = false;

    // const catlogParams = {
    //   //TableName: process.env.PRODUCT_CATALOG_TABLE_NAME,
    //   // TableName: "product-catalog",
    //   TableName: process.env.PRODUCT_CATALOG,
    //   Item: marshall(catlogRequest || {})  
    // };

    // const catlogCreateResult = await ddbClient.send(new PutItemCommand(catlogParams));
    // console.log(catlogCreateResult)
